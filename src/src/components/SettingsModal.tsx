...
const FIELD_DEFINITIONS: FieldDefinition[] = [
  ...
  },
  {
    ...
    tab: 'asr',
    label: '识别模型',
    description: '选择本地语音识别模型。',
    helpText: 'SenseVoice：轻量、CPU 即可、中英日韩粤多语种；Whisper large-v3-turbo：精度更高，建议配合 GPU 使用。切换后会重建识别器。',
    kind: 'select',
    fullWidth: true,
    options: [
      { value: 'whisper-turbo', label: 'Whisper large-v3-turbo（高精度，建议 GPU）' },
      { value: 'sense-voice', label: 'SenseVoice（轻量，CPU 友好）' },
    ],
  },
  {
    key: 'asr_provider',
    tab: 'asr',
    label: '推理设备',
    description: '识别模型运行在 CPU 还是 NVIDIA GPU。',
    helpText: 'GPU(CUDA) 需本机已配置 CUDA 12 + cuDNN 9。设备/库缺失时会回退到 CPU。切换后会重建识别器。',
    kind: 'select',
    fullWidth: true,
    options: [
